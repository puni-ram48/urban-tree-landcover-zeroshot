#!/usr/bin/env python3
"""
Experiment 06: Multi-scale segmentation (300+225 DPI only, fine-tuned).
Tests two-scale fusion without 150 DPI to avoid edge artifacts.
Processes 300+225 DPI images, reprojects to 225 DPI via PGW, fuses masks.
Input: {devEUI}_1.png (300), {devEUI}_2.png (225)
...
# Scale suffix -> scale name mapping (225 + 150 DPI only)
SUFFIX_TO_SCALE = {
    "_1.png": "high",
    "_2.png": "medium"  # 225 DPI  <-- fusion referenc   
}

REQUIRED_SCALES = ["high","medium"]

# Processing
def process_dataset(images_dir: str, output_dirs: dict,
                    logger, sam_generator) -> list:
    """Run two-scale SAM segmentation (300+225 DPI), fuse masks to 225 DPI reference."""
    all_images = []
    for pattern in config.EXP6_MULTISCALE_PATTERNS.values():
        ...
        try:
            ...
            with utils.Timer() as fusion_timer:
                fused_masks = utils.fuse_multiscale_masks(
                    ...
                    min_pixels=config.EXP6_PARAMS["fusion_min_pixels"],
                    merge_threshold=config.EXP6_PARAMS["fusion_merge_threshold"],
                )
...
# Main
def main():
    params = config.EXP6_PARAMS

    print("=" * 70)
    print("EXPERIMENT 06: MULTI-SCALE SEGMENTATION — 300 + 225 DPI")
    print("=" * 70)
    ...
    logger.info("=" * 70)
    logger.info("EXPERIMENT 06: MULTI-SCALE SEGMENTATION — 300+ 225 DPI")
    logger.info("=" * 70)
