#!/usr/bin/env python3
"""
Experiment 05: Multi-scale segmentation (225+150 DPI only, fine-tuned).
Tests two-scale fusion without 300 DPI to avoid edge artifacts.
Processes 225+150 DPI images, reprojects to 225 DPI via PGW, fuses masks.
Input: {devEUI}_2.png (225), {devEUI}_3.png (150)
...
REQUIRED_SCALES = ["medium", "low"]

# Processing
def process_dataset(images_dir: str, output_dirs: dict,
                    logger, sam_generator) -> list:
    """Run two-scale SAM segmentation (225+150 DPI), fuse masks to 225 DPI reference."""
    all_images = []
    for pattern in config.EXP5_MULTISCALE_PATTERNS.values():
        ...
        try:
            ...
            with utils.Timer() as fusion_timer:
                fused_masks = utils.fuse_multiscale_masks(
                    ...
                    min_pixels=config.EXP5_PARAMS["fusion_min_pixels"],
                    merge_threshold=config.EXP5_PARAMS["fusion_merge_threshold"],
                )
...
# Main
def main():
    params = config.EXP5_PARAMS

    print("=" * 70)
    print("EXPERIMENT 05: MULTI-SCALE SEGMENTATION — 225 + 150 DPI")
    print("=" * 70)
    ...
    logger.info("=" * 70)
    logger.info("EXPERIMENT 05: MULTI-SCALE SEGMENTATION — 225 + 150 DPI")
    logger.info("=" * 70)
